...
//输入数据，byte[] 调用方传入
//数据长度int 调用方传入
//输出数据，byte[] 下面三个参数都封装在AudioProcess中,便于使用
//临时输入 float[]
//临时输出 float[]
extern "C" {
JNIEXPORT void JNICALL
Java_com_a78_audio_jni_AudioGraphJni_process(JNIEnv *env, jobject instance, jfloat ratio, jbyteArray in_, jbyteArray out_, jint size, jint sampleRate, jfloatArray floatInput_, jfloatArray floatOutput_) {
    jbyte *in = env->GetByteArrayElements(in_, NULL);
    jbyte *out = env->GetByteArrayElements(out_, NULL);
    jfloat *floatInput = env->GetFloatArrayElements(floatInput_, NULL);
    jfloat *floatOutput = env->GetFloatArrayElements(floatOutput_, NULL);
    ...
    for (i = 0; i < size; i += 2) {
        ...
        int frame = (hi << 8) + lo;
        //除法运算比位运算慢
        floatInput[i >> 2] = (signed short) frame;
    }
    ...
    env->ReleaseFloatArrayElements(floatInput_, floatInput, 0);
    env->ReleaseFloatArrayElements(floatOutput_, floatOutput, 0);
}
}